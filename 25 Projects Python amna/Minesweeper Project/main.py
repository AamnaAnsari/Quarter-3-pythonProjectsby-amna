...
class Board:
    def __init__(self, dim_size, num_bombs):
        ...
        self.dug = set()  # set of places we've dug
    ...
    def dig(self, row, col):
        # true = success wrna falso hua to bomb dig hoga
        ...
        # Recursive dig if current cell == 0
        for r in range(max(0, row-1), min(self.dim_size, row+2)):
            for c in range(max(0, col-1), min(self.dim_size, col+2)):
                if (r, c) in self.dug:
                    continue
    ...
    def __str__(self):
        ...
        for idx in range(self.dim_size):
            ...
            widths.append(
                len(
                    max(columns, key=lambda x: len(x))
                )
            )

        indices_row = '   '
        cells = []
        for idx, col in enumerate(range(self.dim_size)):
